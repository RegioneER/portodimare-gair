...
http://{public_fqdn}/ >> {override_fn}".format(**envs), pty=True)
    ...
    try:
        current_allowed = ast.literal_eval(os.getenv('ALLOWED_HOSTS') or \
                                           "['{public_fqdn}', '{public_host}', 'localhost', 'django', 'porto_di_mare',]".format(**envs))
    except ValueError:
        current_allowed = []
...
--settings={0}".format(_localsettings()), pty=True)
    ctx.run("python manage.py loaddata /usr/src/porto_di_mare/fixtures/initial_data.json \
--settings={0}".format(_localsettings()), pty=True)
...
def _localsettings():
    settings = os.getenv('DJANGO_SETTINGS_MODULE', 'porto_di_mare.settings')
    return settings
