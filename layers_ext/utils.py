...
from base_ext.models import DomainArea, DataPortal, ValidationLevel
...
def resolve_domain_area(domain_area):
    domain_area_resolved = []
    domain_area_unresolved = []
    if domain_area:
        if len(domain_area) > 0:
            for da in domain_area:
                try:
                    da_resolved = DomainArea.objects.get(
                        Q(gn_description__iexact=da) | Q(identifier__iexact=da))
                    domain_area_resolved.append(da_resolved)
                except ObjectDoesNotExist:
                    domain_area_unresolved.append(da)

    return domain_area_resolved, domain_area_unresolved

def is_7zfile(filepath):
...
def un7z_file(upload_file, tempdir=None):
    """
...
def file_upload(filename,
    ...
    if spatial_representation_type is not None:
        try:
            spatial_representation_types = SpatialRepresentationType.objects.filter(
                Q(identifier__iexact=spatial_representation_type) |
                Q(gn_description__iexact=spatial_representation_type))
            if len(spatial_representation_types) == 1:
                spatial_representation_type = spatial_representation_types[0]
            else:
                spatial_representation_type = None
        except BaseException:
            pass
    ...
    if data_portal is not None:
        try:
            data_portals = DataPortal.objects.filter(
                Q(identifier__iexact=data_portal) |
                Q(gn_description__iexact=data_portal))
            if len(data_portals) == 1:
                data_portal = data_portals[0]
            else:
                data_portal = None
        except BaseException:
            pass
    ...
    if validation_level is not None:
        try:
            validation_levels = ValidationLevel.objects.filter(
                Q(identifier__iexact=validation_level) |
                Q(gn_description__iexact=validation_level))
            if len(validation_levels) == 1:
                validation_level = validation_levels[0]
            else:
                validation_level = None
        except BaseException:
            pass
    ...
    if 'xml' in files:
        ...
        identifier, vals, regions, keywords_mt = set_metadata(xml_file)
    ...
    keywords.extend(keywords_mt)

    regions_resolved, regions_unresolved = resolve_regions(regions)
    keywords.extend(regions_unresolved)

    domain_area_resolved, domain_area_unresolved = resolve_domain_area(domain_area)
    keywords.extend(domain_area_unresolved)
    ...
    if not created and overwrite:
        ...
        defaults['supplemental_information'] = defaults.get('supplemental_information', None) or layer.supplemental_information

        defaults['temporal_extent_start'] = defaults.get('temporal_extent_start', None) or layer.temporal_extent_start
        defaults['temporal_extent_end'] = defaults.get('temporal_extent_end', None) or layer.temporal_extent_end
        ...
        defaults['spatial_representation_type'] = defaults.get('spatial_representation_type', None) or layer.spatial_representation_type

        defaults['data_portal'] = defaults.get('data_portal', None) or layer.data_portal

        defaults['validation_level'] = defaults.get('validation_level', None) or layer.validation_level

        defaults['data_accessibility'] = defaults.get('data_accessibility', None) or layer.data_accessibility 

        defaults['starred'] = defaults.get('starred', None) or layer.starred  
    ...
    # Assign the domain area (needs to be done after saving)
    domain_area_resolved = list(set(domain_area_resolved))
    if domain_area_resolved:
        if len(domain_area_resolved) > 0:
            if not layer.domain_area:
                layer.domain_area = domain_area_resolved
            else:
                layer.domain_area.clear()
                layer.domain_area.add(*domain_area_resolved)
    ...
    if defaults.get('supplemental_information', supplemental_information) is not None:
        to_update['supplemental_information'] = defaults.get('supplemental_information', supplemental_information)

    if defaults.get('temporal_extent_start', temporal_extent_start) is not None:
        to_update['temporal_extent_start'] = defaults.get('temporal_extent_start', temporal_extent_start)
    if defaults.get('temporal_extent_end', temporal_extent_end) is not None:
        to_update['temporal_extent_end'] = defaults.get('temporal_extent_end', temporal_extent_end)
    ...
    if defaults.get('spatial_representation_type', spatial_representation_type) is not None:
        to_update['spatial_representation_type'] = defaults.get('spatial_representation_type', spatial_representation_type)
        
    if defaults.get('data_portal', data_portal) is not None:
        to_update['data_portal'] = defaults.get('data_portal', data_portal)
    if defaults.get('validation_level', validation_level) is not None:
        to_update['validation_level'] = defaults.get('validation_level', validation_level)
    if defaults.get('data_accessibility', data_accessibility) is not None:
        to_update['data_accessibility'] = defaults.get('data_accessibility', data_accessibility)
    if defaults.get('starred', starred) is not None:
        to_update['starred'] = defaults.get('starred', starred)    
...
def upload(incoming, user=None, overwrite=False,
           name=None, title=None, abstract=None, supplemental_information=None, spatial_representation_type=None, date=None,
           temporal_extent_start=None, temporal_extent_end=None, license=None,
           category=None, keywords=None, regions=None, 
           domain_area=None, data_portal=None, validation_level=None, data_accessibility=None, starred=None,
    ...
    if os.path.isfile(incoming):
        ...
        if extension in ['.tif', '.shp', '.tar', '.zip', '.7z']:
    ...
    for i, file_pair in enumerate(potential_files):
        ...
        if save_it:
            try:
                ...
                if is_7zfile(filename):
                    file_zip = filename
                    filename = un7z_file(filename)
