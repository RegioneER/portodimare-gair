...
from layers_ext.forms import LayerForm
...
from geonode.utils import (resolve_object,
                           ...
                           GXPLayer,
                           GXPMap)
from layers_ext.utils import file_upload, is_raster, is_vector
...
def _resolve_layer_by_id(request, alternate, permission='base.view_resourcebase',
                   msg=_PERMISSION_MSG_GENERIC, **kwargs):
    """
    Resolve the layer by the provided typename (which may include service name) and check the optional permission.
    """
    ...
    if Service.objects.filter(id=alternate):
        service = Service.objects.filter(id=alternate)
        return resolve_object(
            request,
            Layer,
            {
                'id': alternate if service[0].method != "C" else alternate
            },
            permission=permission,
            permission_msg=msg,
            **kwargs)
...
def layer_metadata(
   ...
   #  ) and tkeywords_form.is_valid():
        ...
        new_domain_area = layer_form.cleaned_data['domain_area']
        #new_domain_area = [x.strip() for x in layer_form.cleaned_data['domain_area']]
        if new_domain_area is not None:
            layer.domain_area.clear()
            layer.domain_area.add(*new_domain_area)
        ...
        layer.category = layer_form.cleaned_data['category'] 
        layer.save()
            ...
            try:
                ...
                if layer.is_vector() and is_raster(base_file):
                    ...
                    saved_layer = file_upload(
                        ...
                        domain_area=list(layer.domain_area.values_list('identifier', flat=True)),
                        # date=layer.date,
                        overwrite=True,
...
def get_layer_by_id(request, id): 
    """Get Layer object as JSON"""
    ...
    logger.debug('Call get layer')
    if request.method == 'GET':
        layer_obj = _resolve_layer_by_id(request, id)
        attributes = []

        for attrset in layer_obj.attribute_set.order_by('display_order'):
            attr = {
                'name': attrset.attribute,
                'type': attrset.attribute_type
            }
        ...
        full_name = (layer_obj.owner.get_full_name() or layer_obj.owner.get_username())

        logger.debug(id)
        response = {
            'id': id,
            'typename': layer_obj.name,
            'name': layer_obj.name,
            'title': layer_obj.title,
            'url': layer_obj.get_tiles_url(),   
            'is_approved': layer_obj.is_approved,   
            'is_published': layer_obj.is_published,   
            'abstract': layer_obj.abstract,
            'thumbnail_url': layer_obj.thumbnail_url,
            'alternate': layer_obj.alternate ,
            'attribute': attributes,
            'is_vector' : layer_obj.is_vector(),
            'owner_name': full_name,
            'owner__username': layer_obj.owner.get_username(),
            'popular_count': layer_obj.popular_count,
            # 'created': layer_obj.created,
            'rating': layer_obj.rating,
        }
        return HttpResponse(json.dumps(
            response,
            ensure_ascii=False,
