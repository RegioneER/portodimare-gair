...
define(function (require, exports) {
    ...
    LayerInfo.prototype.displayUploadedLayerLinks = function(resp) {
        ...
        var a = '<a href="' + resp.url + '" class="btn btn-info">' + gettext(resourceType.capitalize() + ' Info') + '</a>';
        var b = '<a href="' + resp.url + '/metadata" class="btn btn-default">' + gettext('Edit Metadata') + '</a>';
        var c = '<a href="' + resp.url.replace(/^\/layers_ext/, '/layers') + '/metadata_upload" class="btn btn-default">' + gettext('Upload Metadata') + '</a>';
        var d = '<a href="' + resp.url.replace(/^\/layers_ext/, '/layers') + '/style_upload" class="btn btn-default">' + gettext('Upload SLD') + '</a>';
        var e = '<a href="' + resp.url.replace(/^\/layers_ext/, '/gs') + '/style/manage" class="btn btn-default">' + gettext('Manage Styles') + '</a>';
    ...
    LayerInfo.prototype.doFinal = function (resp) {
        ...
        if (resp.hasOwnProperty('redirect_to') && resp.redirect_to.indexOf('/upload/final') > -1) {
            ...
            var a = '<a id="' + element + '" class="btn btn-info" target="_new">Continue</a>';
    ...
    LayerInfo.prototype.displayFiles = function () {
        ...
        $.each(this.files, function (idx, file) {
            ...
            a.attr('class', 'btn btn-danger fa fa-remove icon-left remove-file');
            a.attr('data-toggle', 'tooltip');
            a.attr('data-placement', 'left');
            a.attr('data-idx', idx);

            li.addClass("padding-sm");
            // console.log("TEST; "); 
            console.log(li); 
            ...
            a.on('click', function (event) {
                ...
                ul.find("li").eq( $(this).attr('data-idx') ).addClass("list-unstyled-selected");
                if (confirm(gettext("Are you sure to remove this file?"))) {
                    var target = $(event.target),
                        layer_info,
                        layer_name = target.data('layer'),
                        file_name  = target.data('file');
                    ...
                    ul.find("li").eq( $(this).attr('data-idx') ).removeClass("list-unstyled-selected");
                }
            });
        });
    };
    ...
    LayerInfo.prototype.displayErrors = function () {
        ...
        $.each(this.errors, function (idx, error) {
            var li = $('<li/>', {text: error, 'class': 'alert alert-danger'});
            li.appendTo(ul);
