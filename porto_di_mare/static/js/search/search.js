...
(function () {
	...
	module.load_modules = function ($http, $rootScope, $location) {
		...
		$http.get(CASESTUDIES_ENDPOINT, { params: params })
			.success(function (data) {
			
				if ($location.search().hasOwnProperty('module')) {
					data.objects = module.set_initial_filters_from_query(data.objects,
						$location.search()['module'], 'identifier');
				}
				var object_arr = [];
				for (var i = 0; i < data.objects.length; i++) {
					object_arr.push(
						{
							'module': data.objects[i].module,
							'cstype': data.objects[i].cstype,
							'active': ''
						}
					);
				}
				...
				const cstype_list = [];
				const map = new Map();
				for (const item of object_arr) {
					if (!map.has(item.module)) {
						map.set(item.module, true);    // set any value to Map
						module_list.push({
							module: item.module,
						});
					}
					if (!map.has(item.cstype)) {
						map.set(item.cstype, true);    // set any value to Map
						cstype_list.push({
							cstype: item.cstype,
						});
					}
				...
				if (CS_LIST)
					setCaseStudiesList(); 
				// $rootScope.modules = module_list;
				// $rootScope.cstype = cstype_list;
			...
			.error(function(err) {
				// alert("Si è verificato un errore, verifica il token"); 
				var alertObj = {};
				if (err.message == null && CS_LIST) {
					
					alertObj["title"] = "Service Unavailable";
					alertObj["message"] = "The server is temporarily unable to service your request due to maintenance downtime or capacity problems.<br />Please try again later.";
				}
	...
	// Load categories, keywords, and regions
	module.load_maincategory = function ($http, $rootScope, $location) {
		var params = typeof FILTER_TYPE == 'undefined' ? {} : { 'type': FILTER_TYPE };
		if ($location.search().hasOwnProperty('title__icontains')) {
			params['title__icontains'] = $location.search()['title__icontains'];
		}
		$http.get(MAINCATEGORIES_ENDPOINT, { params: params }).success(function (data) {
			if ($location.search().hasOwnProperty('category__main_category__identifier__in')) {
				data.objects = module.set_initial_filters_from_query(data.objects,
					$location.search()['category__main_category__identifier__in'], 'identifier');
			}
			$rootScope.maincategory = data.objects;
			if (HAYSTACK_FACET_COUNTS && $rootScope.query_data) {
				module.haystack_facets($http, $rootScope, $location);
			}
		});
	...
	// Load domainarea, dataportal, validation_level

	module.load_domainarea = function ($http, $rootScope, $location) {
		var params = typeof FILTER_TYPE == 'undefined' ? {} : { 'type': FILTER_TYPE };
		$http.get(DOMAINAREAS_ENDPOINT, { params: params }).success(function (data) {
			if ($location.search().hasOwnProperty('domain_area__identifier__in')) {
				data.objects = module.set_initial_filters_from_query(data.objects,
					$location.search()['domain_area__identifier__in'], 'identifier');
			}
			$rootScope.domainarea = data.objects;
			if (HAYSTACK_FACET_COUNTS && $rootScope.query_data) {
				module.haystack_facets($http, $rootScope, $location);
			}
		});
	...
	module.load_dataportal = function ($http, $rootScope, $location) {
		var params = typeof FILTER_TYPE == 'undefined' ? {} : { 'type': FILTER_TYPE };
		$http.get(DATAPORTALS_ENDPOINT, { params: params }).success(function (data) {
			if ($location.search().hasOwnProperty('data_portal__identifier__in')) {
				data.objects = module.set_initial_filters_from_query(data.objects,
					$location.search()['data_portal__identifier__in'], 'identifier');
			}
			$rootScope.dataportal = data.objects;
			if (HAYSTACK_FACET_COUNTS && $rootScope.query_data) {
				module.haystack_facets($http, $rootScope, $location);
			}
		});
	...
	module.load_validationlevel = function ($http, $rootScope, $location) {
		var params = typeof FILTER_TYPE == 'undefined' ? {} : { 'type': FILTER_TYPE };
		$http.get(VALIDATIONLEVELS_ENDPOINT, { params: params }).success(function (data) {
			if ($location.search().hasOwnProperty('validation_level__identifier__in')) {
				data.objects = module.set_initial_filters_from_query(data.objects,
					$location.search()['validation_level__identifier__in'], 'identifier');
			}
			$rootScope.validationlevel = data.objects;
			if (HAYSTACK_FACET_COUNTS && $rootScope.query_data) {
				module.haystack_facets($http, $rootScope, $location);
			}
		});
	...
	module.haystack_facets = function ($http, $rootScope, $location) {
		...
		if ("data_portal" in $rootScope) {
			try {
				$rootScope.data_portal_counts = data.meta.facets.data_portal;
				for (var id in $rootScope.data_portal) {
					var data_portal = $rootScope.data_portal[id];
					if (data_portal.identifier in $rootScope.data_portal_counts) {
						data_portal.count = $rootScope.data_portal_counts[data_portal.identifier]
					} else {
						data_portal.count = 0;
					}
				}
		...
		if ("validation_level" in $rootScope) {
			try {
				$rootScope.validation_level_counts = data.meta.facets.validation_level;
				for (var id in $rootScope.validation_level) {
					var validation_level = $rootScope.validation_level[id];
					if (validation_level.identifier in $rootScope.validation_level_counts) {
						validation_level.count = $rootScope.validation_level_counts[validation_level.identifier]
					} else {
						validation_level.count = 0;
					}
				}
			} catch (err) {
				// console.log(err);
		...
		if ("domain_area" in $rootScope) {
			try {
				$rootScope.domain_area_counts = data.meta.facets.domain_area;
				for (var id in $rootScope.domain_area) {
					var domain_area = $rootScope.domain_area[id];
					if (domain_area.name in $rootScope.domain_area_counts) {
						domain_area.count = $rootScope.domain_area_counts[domain_area.name]
					} else {
						domain_area.count = 0;
					}
				}
	...
	*/
	module.run(function ($http, $rootScope, $location) {
		...
		*/
		...
		if ($('#modules').length > 0) {
			module.load_modules($http, $rootScope, $location);
			/* var a = $('#modules a');
			a.click(function(e) {
				e.stopImmediatePropagation();
				var v = $(this).attr("data-value");
				FILTER_MODULE = v;
				module.constant('Configs', {url: SEARCH_URL});
			}); */ 
		}
		...
		if ($('#maincategory').length > 0) {
			module.load_maincategory($http, $rootScope, $location);
		}
		...
		if ($('#domainarea').length > 0) {
			module.load_domainarea($http, $rootScope, $location);
		}
		if ($('#validationlevel').length > 0) {
			module.load_validationlevel($http, $rootScope, $location);
		}
		if ($('#dataportal').length > 0) {
			module.load_dataportal($http, $rootScope, $location);
		}
	...
	*/
	module.controller('geonode_search_controller', function ($injector, $scope, $location, $http, Configs) {
		...
		*/
		...
		$scope.single_choice_listener_remove = function ($event) {
			var element = $($event.currentTarget);
			var data_filter = element.attr('data-filter');
		...
		*/
		...
		$('#clear-search').click(function () {
			...
			query_api($scope.query);
			$( "a" ).removeClass( "btn-clear-link active" ).addClass( "btn-clear-link" );
		});
