...
from api_ext import (TagResource,
                  RegionResource,
                  OwnersResource,
...
class CommonMetaApi:
    ...
    filtering = {'title': ALL,
                 ...
                 'domain_area': ALL_WITH_RELATIONS,
                 'data_portal': ALL_WITH_RELATIONS,
                 'validation_level': ALL_WITH_RELATIONS,
                 'group': ALL_WITH_RELATIONS,
                 'owner': ALL_WITH_RELATIONS,
                 'starred': ALL_WITH_RELATIONS,
                 'date': ALL,
                 }
    ordering = ['date', 'title', 'popular_count', 'starred']
    max_limit = None
...
class CommonModelApi(ModelResource):
    ...
    group = fields.ToOneField(
        ...
        null=True,
        full=True)
    domain_area = fields.ToManyField(
        DomainAreaResource,
        'domain_area',
        null=True)
    data_portal = fields.ToOneField(
        DataPortalResource,
        'data_portal',
        null=True)
    validation_level = fields.ToOneField(
        ValidationLevelResource,
        'validation_level',
        null=True)
    ...
    VALUES = [
        ...
        'bbox_y0',
        'bbox_y1',
        'category__main_category__gn_description',
        ...
        'is_published',
        'dirty_state',
        'data_portal__gn_description',
        'data_accessibility',
        'validation_level__gn_description',
        'starred',
    ]
    ...
    def build_haystack_filters(self, parameters):
        ...
        main_category = parameters.getlist("category__main_category__identifier__in")
        ...
        # Data portal
        data_portal = parameters.getlist("data_portal__identifier__in")

        # Domain area
        domain_area = parameters.getlist("domain_area__identifier__in")

        # Validation level
        validation_level = parameters.getlist("validation_level__identifier__in")

        # Starred
        starred = parameters.getlist("starred")
        ...
        if category:
            for ca in category:
                sqs = (
                    SearchQuerySet() if sqs is None else sqs).filter_or(
                    category_exact__exact=ca)
        
        # filter by data_portal
        if data_portal:
            sqs = (SearchQuerySet() if sqs is None else sqs).narrow(
                'data_portal:%s' % ','.join(map(str, data_portal)))
        
        # filter by domain_area
        if domain_area:
            sqs = (SearchQuerySet() if sqs is None else sqs).narrow(
                'domain_area:%s' % ','.join(map(str, domain_area)))
                
        # filter by validation_level
        if validation_level:
            sqs = (SearchQuerySet() if sqs is None else sqs).narrow(
                'validation_level:%s' % ','.join(map(str, validation_level)))   

        # filter by starred
        if starred:
            sqs = (SearchQuerySet() if sqs is None else sqs).narrow(
                'starred:%s' % ','.join(map(str, starred)))         
        ...
        if not settings.SKIP_PERMS_FILTER:
            ...
            if len(filter_set) > 0:
                sqs = sqs.filter(id__in=filter_set_ids).facet('type').facet('subtype').facet(
                    'owner') .facet('keywords').facet('regions').facet('category').facet(
                        'data_portal').facet('domain_area').facet('validation_level').facet('main_category').facet('starred')
            else:
                sqs = None
        else:
            sqs = sqs.facet('type').facet('subtype').facet(
                'owner').facet('keywords').facet('regions').facet(
                    'category').facet('data_portal').facet('domain_area').facet('validation_level').facet('main_category').facet('starred')
    ...
    def format_objects(self, objects):
        ...
        for obj in objects:
            ...
            if obj.category:
                ...
                if obj.category.main_category:
                    formatted_obj['category_main_category_gn_description'] = obj.category.main_category.gn_description
            if obj.group:
            ...
            if obj.data_portal:
                formatted_obj['data_portal'] = obj.data_portal.gn_description
            if obj.validation_level:
                formatted_obj['validation_level'] = obj.validation_level.gn_description
            formatted_obj['domain_area'] = [d.gn_description for d in obj.domain_area.all()] if obj.domain_area else []
...
class DocumentResource(CommonModelApi):
    ...
    def format_objects(self, objects):
        ...
        for obj in objects:
            ...
            if obj.category:
                formatted_obj['category__gn_description'] = obj.category.gn_description
                formatted_obj['category__main_category__gn_description'] = obj.category.main_category.gn_description
            if obj.group:
                formatted_obj['group'] = obj.group
                try:
                    formatted_obj['group_name'] = GroupProfile.objects.get(slug=obj.group.name)
