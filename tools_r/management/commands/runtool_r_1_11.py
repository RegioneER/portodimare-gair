...
from tools_r.models import ToolsR_1_11
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            '--directory',
            dest="directory",
            default="/mnt/volumes/statics/static/script_r/1.11/",
        ...
        parser.add_argument(
            ...
            '--output',
            dest="output",
            default="/mnt/volumes/statics/uploaded/tools_r/1.11/",
    ...
    def handle(self, *args, **options):
        ...
        if os.path.exists(directory):
            ...
            if not os.path.exists(dir_output):
                ...
                if os.path.isfile(directory+'pairwise_matrix.csv'):
                    shutil.copy2(directory+'pairwise_matrix.csv', dir_output)
                if os.path.isfile(directory+'Fishing_Gear.csv'):
                    shutil.copy2(directory+'Fishing_Gear.csv', dir_output)
            ...
            for c in criteria.split(','):
                ...
                if (len(temp)>1):
                    if (temp[1] == "no_take"):
                        no_take = "1.11/"+dir_temp+layer.name+".tif"
                        extent = str(layer.bbox_x0)+" "+str(layer.bbox_x1)+" "+str(layer.bbox_y0)+" "+str(layer.bbox_y1)
                    else:
                        geotiff += "1.11/"+dir_temp+layer.name+".tif "
                else:
                    coastLine = layer.name
            
            # stringa per lo script
            data = " 1.11/apply_fprmcda_ssf_comdline.R 1.11/"+dir_temp+" 1.11/"+dir_temp+"output/ "+name+" "+extent+" "+coastLine+" 1.11/components_cuts_grades.csv "+dir_output+"/pairwise_matrix.csv "+dir_output+"/Fishing_Gear.csv "+no_take+" "+geotiff+"\n"
            ...
            if (received == '0'):
                ...
                if (id_tool):
                    tr = ToolsR_1_11.objects.filter(id=id_tool).update(status="completed")
            else:
