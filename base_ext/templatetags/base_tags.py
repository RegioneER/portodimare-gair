...
def facets(context):
    ...
    main_category_filter = request.GET.getlist('category__main_category__identifier__in', None)
    ...
    data_portal_filter = request.GET.getlist('data_portal__identifier__in', None)
    validation_level_filter = request.GET.getlist('validation_level__identifier__in', None)
    domain_area_filter = request.GET.getlist('domain_area__identifier__in', None)
    starred_filter = request.GET.getlist('starred__in', None)
    ...
    if facet_type == 'documents':
        ...
        if main_category_filter:
            documents = documents.filter(category__main_category__identifier__in=main_category_filter)
        if regions_filter:
        ...
        if data_portal_filter:
            documents = documents.filter(data_portal__identifier__in=data_portal_filter)
        if validation_level_filter:
            documents = documents.filter(validation_level__identifier__in=validation_level_filter)
        if domain_area_filter:
            documents = documents.filter(domain_area__identifier__in=domain_area_filter)
        if starred_filter:
            documents = documents.filter(starred__in=starred_filter)
        ...
        if main_category_filter:
            layers = layers.filter(category__main_category__identifier__in=main_category_filter)
        if regions_filter:
        ...
        if data_portal_filter:
            layers = layers.filter(data_portal__identifier__in=data_portal_filter)
        if validation_level_filter:
            layers = layers.filter(validation_level__identifier__in=validation_level_filter)
        if domain_area_filter:
            layers = layers.filter(domain_area__identifier__in=domain_area_filter)
        if starred_filter:
            layers = layers.filter(starred__in=starred_filter)
        ...
        if category_filter:
            ...
            documents = documents.filter(category__identifier__in=category_filter)
        if main_category_filter:
            maps = maps.filter(category__main_category__identifier__in=main_category_filter)
            documents = documents.filter(category__main_category__identifier__in=main_category_filter)
        if regions_filter:
        ...
        if date_range_filter:
            ...
            documents = documents.filter(date__range=date_range_filter.split(','))
        if data_portal_filter:
            maps = maps.filter(data_portal__identifier__in=data_portal_filter)
            documents = documents.filter(data_portal__identifier__in=data_portal_filter)
        if validation_level_filter:
            maps = maps.filter(validation_level__identifier__in=validation_level_filter)
            documents = documents.filter(data_portal__identifier__in=validation_level_filter)
        if domain_area_filter:
            maps = maps.filter(domain_area__identifier__in=domain_area_filter)
            documents = documents.filter(domain_area__identifier__in=domain_area_filter)
        if starred_filter:
            maps = maps.filter(starred__in=starred_filter)
            documents = documents.filter(starred__in=starred_filter)
