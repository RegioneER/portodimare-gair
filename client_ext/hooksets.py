...
from geonode_mapstore_client.hooksets import MapStoreHookSet
...
class LeafletHookSet(MapStoreHookSet):

    #CASE STUDIES
    def casestudies_detail_template(self, context=None):
        return 'leaflet/casestudies_leaflet_map.html'
    ...
    def map_new_template(self, context=None):
        ...
        if self.isEditLayer(context):
            return 'mapstore/layer_edit.html'
        elif self.isViewLayer(context):
            return 'mapstore/layer_view.html'
        else:
            return 'mapstore/map_new.html'

    # LAYER DETAIL (MAP STORE)
    def layer_detail_template(self, context=None):
        self.initialize_context(context, callback=ms2_config_converter.convert)
        return 'mapstore/layer_map.html'

    # MAP DETAIL (MAP STORE)
    def map_detail_template(self, context=None):
        self.initialize_context(context, callback=ms2_config_converter.convert)
        return 'mapstore/map_detail.html'