...
from base_ext.models import DataPortal, ValidationLevel, DomainArea
...
class TreeWidget(TaggitWidget):
    ...
    def render(self, name, value, attrs=None):
        ...
        output.append(
            '<span class="input-group-addon" id="treeview-toggle"><span class="fa fa-folder"></span></span>')
...
class DomainAreaMultipleChoiceField(forms.MultipleChoiceField):
...
class CategoryChoiceFieldGroup(forms.ModelChoiceField):
    def __init__(self, *args, **kwargs):
        super(CategoryChoiceFieldGroup, self).__init__(*args, **kwargs)
        # ******
        # ALERT -> hack: in caso di prima installazione commentare queste righe, da sistemare...
        groups = groupby(kwargs['queryset'], attrgetter('main_category'))
        self.choices = [(main_category, [(c.id, self.label_from_instance(c)) for c in category])
                        for main_category, category in groups]
        # *******

class CategoryForm(forms.Form):
    category_choice_field = CategoryChoiceFieldGroup(
        required=False,
        label='*' + _('Category'),
        empty_label=None,
...
class ResourceBaseForm(TranslationModelForm):
    ...
    category = CategoryChoiceFieldGroup(
        required=False,
        label= _('Category'),
        empty_label=None,
        queryset=TopicCategory.objects.filter(
            is_choice=True) .extra(
            order_by=['main_category','description']))
    ...
    data_portal = forms.ModelChoiceField(
        label=_("Data portal"),
        required=False,
        queryset=DataPortal.objects.all())

    validation_level = forms.ModelChoiceField(
        label=_("Validation Level"),
        required=False,
        queryset=ValidationLevel.objects.all())

    data_accessibility = forms.CharField(
        label=_("Data accessibility"),
        required=False)
    
    domain_area = forms.ModelMultipleChoiceField(
        label=_("Domain area"),
        required=False,
